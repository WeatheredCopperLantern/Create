...
public class LinkedControllerItemRenderer extends CustomRenderedItemModelRenderer {
	...
	static void tick() {
		...
		for (int i = 0; i < LinkedControllerItemRenderer.buttons.size(); i++) {
			...
			lerpedFloat.chase(LinkedControllerClientHandler.currentlyPressed.contains(i) ? 1 : 0, 0.4f, LerpedFloat.Chaser.EXP);
	...
	protected static void render(final ItemStack stack, final CustomRenderedItemModel model, final PartialItemModelRenderer renderer, final ItemDisplayContext transformType, final PoseStack ms, int light, final RenderType renderType, boolean active, boolean renderDepression) {
		...
		if (renderType == RenderType.NORMAL) {
			...
			active = false;
			final boolean noControllerInMain = true;//!AllItems.LINKED_CONTROLLER.isIn(mc.player.getMainHandItem());
