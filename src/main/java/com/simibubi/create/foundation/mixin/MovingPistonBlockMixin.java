...
public class MovingPistonBlockMixin {
	...
	private List<ItemStack> create$dropBEItems(final BlockState instance, final LootParams.Builder builder, final Operation<List<ItemStack>> original, @Local final PistonMovingBlockEntity pistonmovingblockentity) {
		if (((PistonMovingBlockEntityAccessor) pistonmovingblockentity).create$getBlockEntity() instanceof final SmartBlockEntity smartBE) {
			Create.PERSISTENT_OBJECTS.getManager(pistonmovingblockentity.getLevel().dimension()).removePersistentBlockBoundObject(pistonmovingblockentity.getBlockPos());
			smartBE.destroy();
		}
